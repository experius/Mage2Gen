...
class ModelSnippet(Snippet):
	...
	**Model ID field**:	The snippet will auto add the model id field to the database table, the field name is <model_name>_id.
	...
	def add(self, model_name, field_name, field_type='text', adminhtml_grid=False, adminhtml_form=False, extra_params=False):
		...
		# add grid 
		if adminhtml_grid:
			self.add_adminhtml_grid(model_name, field_name, model_table, model_id, collection_model_class)

		if adminhtml_form:
			self.add_adminhtml_form(model_name, field_name, model_table, model_id, collection_model_class, model_class)

	def add_adminhtml_grid(self, model_name, field_name, model_table, model_id, collection_model_class):
	...
	def add_adminhtml_form(self, model_name, field_name, model_table, model_id, collection_model_class, model_class):
		frontname = self.module_name.lower()
		# Add block buttons
		# Back button
		...
		# link controller
		link_controller = Phpclass('Controller\\Adminhtml\\' + model_name.replace('_', ''), extends='\\Magento\\Backend\\App\\Action',
